...
// make device not switch from powered host mode to self-powered OTG mode
//#define FIXED_INSTALLATION_MODE  
...
static int smb347_configure_charger(struct i2c_client *client, int value)
{
	...
	if (value) {
		printk("smb347_configure_charger enable external power\n");
		...
		*/
		...
		printk("smb347_configure_charger disable external power\n");
		ret = smb347_read(client, smb347_CMD_REG);
		if (ret < 0) {
			dev_err(&client->dev, "%s: err %d\n", __func__, ret);
			goto error;
...
static void smb347_otg_status(enum usb_otg_state to, enum usb_otg_state from, void *data)
{
	...
	printk("smb347_otg_status to=%d from=%d lastOtgState=%d lastExternalPowerState=%d lastChargeSlaveDevicesState=%d\n",
	...
	if (to == OTG_STATE_A_HOST) {
		...
		if(data!=NULL) {
			// not called by inok_isr_work_function()
			if(charger->cur_cable_type!=1 && charger->cur_cable_type!=3) {
				// no power
				printk("smb347_otg_status no external power\n");
				if(lastExternalPowerState>0) {
					printk("smb347_otg_status lost external power just now\n");
				} else {
					// still no power - wait up to 1300 ms for external power detection
					printk("smb347_otg_status wait for external power detection ...\n");
					...
					printk("smb347_otg_status wait for external power detection done\n");
				}
			}
		}
		if(charger->cur_cable_type==1 || charger->cur_cable_type==3)
			newExternalPowerState = 1;
		//printk("smb347_otg_status state=%d cable_type=%d newExternalPowerState=%d\n",
		//  charger->state,charger->cur_cable_type,newExternalPowerState);

		if(newExternalPowerState != lastExternalPowerState) {
			/* configure charger: enable or disable charging of internal battery */
			printk("smb347_otg_status change use of external power to %d\n",newExternalPowerState);
		...
		// enable host-mode and configure slave-charging (depending of new external power state)
		...
		if(from == OTG_STATE_A_HOST) {
			// disable host-mode and slave-charging
		...
		if(newExternalPowerState && !lastExternalPowerState) {
			/* configure charger: enable charging of internal battery */
			printk("smb347_otg_status enable external charge\n");
...
#endif
					} else if(retval == APSD_HOST_MODE_CHARGING) {
...
static void inok_isr_work_function(struct work_struct *dat)
{
	...
	printk("inok_isr_work_function power loss/gain lastOtgState=%d lastExternalPowerState=%d lastChargeSlaveDevicesState=%d\n",
		lastOtgState,lastExternalPowerState,lastChargeSlaveDevicesState);
...
#ifdef FIXED_INSTALLATION_MODE
		printk("inok_isr_work_function lost external power in host mode; not trying to continue host mode by self-charging slave devices\n");
#else
		printk("inok_isr_work_function lost external power in host mode; trying to continue host mode by self-charging slave devices\n");
		// normally, smb347_otg_status() is called whenever the OTG adapter is pulled or plugged
		// but here, external power was lost while the OTG adapter remains plugged
		// we call smb347_otg_status() now, to activate self-charging of slave devices to continue host mode in std OTG mode 
		// NOTE: if we would NOT call smb347_otg_status() here, slave devices would be without power now
		//if(autoSwitchSelfPoweredHostMode)
	...
	cable_type_detect();
	if(charger->cur_cable_type!=1 && charger->cur_cable_type!=3) {
		// no power is currently available
		if(lastExternalPowerState>0) {
			printk("inok_isr_work_function lost external power just now\n");
		} else {
			// we did not have power before, so it might come in now...
			// the cable_type_detect check above MAY have come too early
			// we wait up to 1300 ms for external power detection
			printk("inok_isr_work_function wait for external power detection ...\n");
			schedule_timeout_interruptible(msecs_to_jiffies(300));
			cable_type_detect();
			if(charger->cur_cable_type!=1 && charger->cur_cable_type!=3) {
				schedule_timeout_interruptible(msecs_to_jiffies(500));
	...
	if(charger->cur_cable_type!=1 && charger->cur_cable_type!=3) {
		...
		printk("inok_isr_work_function no power: cancel charger events\n");
		cancel_delayed_work(&charger->curr_limit_work);
		cancel_delayed_work(&charger->inok_isr_work);
		smb347_clear_interrupts(client);	// fixme?
	...
	// power is incoming
	printk("inok_isr_work_function external power available\n");
}
